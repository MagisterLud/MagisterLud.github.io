...
> ### Theorem (Mandell'01)
> Let $k$ be $\mathbb{F}_p$. Two nilpotent $p$ complete spaces $X$ and $Y$ of finite $p$ type are weakly homotopy equivalent if and only if 
> <div class="math"> 
> $$C^{\ast}(X, k){\simeq}_{E_{\infty}} C^{\ast}(Y, k),$$ 
> </div> 
...
<div class="math"> 
$$C^{\ast}(X, k)\simeq_{E_\infty}C^{\ast}(Y, k)$$ 
</div> 
