 ...
 **Theorem (Mandell'01)** Two nilpotent $p$ complete spaces $X$ and $Y$ of finite $p$ type are weakly homotopy equivalent if and only if $C^{\ast}(X, {\mathbb{F}}_{p}){\simeq}_{E_{\infty}} C^{\ast}(Y, \mathbb{F}_{p})$, i. e., their singular cochain complexes with ${\mathbb{F}}_{p}$ coefficients are weakly equivalent as ${E}_{\infty}$ algebras.
