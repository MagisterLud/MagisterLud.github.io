...
> ### Theorem (Mandell'01)
> Let $k$ be $\mathbb{F}_p$. Two nilpotent $p$ complete spaces $X$ and $Y$ of finite $p$ type are weakly homotopy equivalent if and only if <div class="math"> $$C^{\ast}(X, k){\simeq}_{E_{\infty}} C^{\ast}(Y, k),$$ </div>  i. e., their singular cochain complexes with $k$ coefficients are weakly equivalent as ${E}_{\infty}$ algebras.

**Sketch of proof.** One direction is clear. For the other one, assume that <div class="math"> $$C^{\ast}(X, k)\simeq_{E_\infty}C^{\ast}(Y, k)$$ </div> as $E_{\infty}$ algebras over $k$. It suffices to consider the case over the algebraic completed field $\overline{k}$ as the map $C^{\ast}(X, k)\otimes_{k}\overline{k}\to C^{\ast}(X, \overline{k})$ is a weak equivalence. 
