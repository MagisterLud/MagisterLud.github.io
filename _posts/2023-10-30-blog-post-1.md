...
>  ```math C^\ast(X, \mathbb{F}_p)\simeq_{E_\infty} C^\ast(Y, \mathbb{F}_p),```
>  i. e., their singular cochain complexes with $\mathbb{F}_p$ coefficients are weakly equivalent as $E_\infty$ algebras.
